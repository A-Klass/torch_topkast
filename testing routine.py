...
def train(net, num_epochs, loss, optimizer, batch_size, boston = datasets.load_boston(), split = [.7, .2, .1], patience = 5):
    ...
    for epoch in range(num_epochs):
        ...
        losses_validation[epoch] = loss(net(validation_dataset[:][0].float(), sparse=False), validation_dataset[:][1].float())
        if (epoch + 1) % 100 == 0:
    ...
    test_loss = loss(net(test_dataset[:][0].float(), sparse=False), test_dataset[:][1].float())
...
class TopKastNet(nn.Module):
    ...
    def forward(self, X, sparse=True):
        ...
        return self.self.hidden(y_1, sparse=sparse)
